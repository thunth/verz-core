...
//					die;
				...
				//Custom by jason, to add relation between menuitem with page
				if (isset($_POST['Menuitem'])) {
					foreach ($_POST['Menuitem'] as $menuItem){
						if ($menuItem['page_id'] > 0) {
							$Item = Menuitem::findByLink($menuItem['link']);
							$page = Page::model()->findByPk($menuItem['page_id']);
							$page->menuitem_id = $Item[0]->id;
							$page->update('menuitem_id');
						}
					}
				}
