...
class Banner extends _BaseModel
{
	...
	public $defineImageSize = array('large_image' => array(array('alias' => 'thumb1', 'size' => '1170x350')));
	...
	public function rules()
	{
		...
		return array(
			array(' banner_title, banner_description, large_image, order_display', 'required'),
			array('place_holder_id,group_banner_id, status, order_display', 'numerical', 'integerOnly' => true),
			array('name', 'length', 'max' => 250),
			array('thumb_image, large_image, link', 'length', 'max' => 255),
			array('banner_title,banner_description,link,group_banner_id', 'required', 'on' => 'create, update'),
			array('id', 'unique', 'on' => 'create, update'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, banner_title, banner_description,group_banner_id, thumb_image, large_image, link, place_holder_id, status, order_display, created_date', 'safe', 'on' => 'search'),
	...
	public function attributeLabels()
	{
		return array(
			...
			'order_display' => Yii::t('translation', 'Display Order'),
			'created_date' => Yii::t('translation', 'Created Date'),
			'group_banner_id' => Yii::t('translation', 'Belong to Banner'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('group_banner_id', $this->group_banner_id, true);
		$criteria->addCondition('group_banner_id=0');
		$sort = new CSort();
	...
	protected function beforeSave()
	{
		if ($this->isNewRecord)
		{
			...
			$this->name = $this->banner_title;
		}
		$this->place_holder_id = 0;
		return true;
	}

	public static function getDropdownbanner()
	{

		$model = GroupBanner::model()->findAll();
		$list = CHtml::listData($model, 'id', 'name');
		return $list;
	}

	public static function getAllBanner($group_id)
	{
		$models = self::model()->findAll(array('condition' => 'group_banner_id = ' . $group_id));
		return $models;
	}
