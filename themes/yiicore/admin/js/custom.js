$(document).ready(function(){
    // MAR 14, 2015  ANH DUNG FIX jQuery .live() has been removed in version 1.9 onwards.
    // http://stackoverflow.com/questions/14354040/jquery-1-9-live-is-not-a-function
    $('body').on('click', '.hidden-symbol', function(){
...
//    $('.showing-symbol').live('click',function(){
    $('body').on('click', '.showing-symbol', function(){
        var menuid = $(this).parent('li').attr('id');
...
function runDatePicker(url, format) {
    ...
    // Dec 23, 2014 ANH DUNG ADD
    $('.my-datepicker-control-search').each(function(){
        var id = $(this).attr('id');
        $('#'+id).datepicker(
            jQuery.extend(
                {showMonthAfterYear:false}, 
